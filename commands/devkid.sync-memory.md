---
name: devkid.sync-memory
description: Update memory-bank with current project state, git history, and task progress
---

# Dev-Kid Sync Memory

Updates memory-bank with current project state, preserving institutional knowledge across sessions and context compression.
...
# Full sync (regenerate all files)
/devkid.sync-memory --full
```
...
# Determine sync type
if [ "$FULL_SYNC" = "--full" ] || [ "$FULL_SYNC" = "true" ]; then
...
**Manual triggers**:
- 📝 User requests: `/devkid.sync-memory`
