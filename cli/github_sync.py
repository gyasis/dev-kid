        ...
        with open(exec_plan_path) as f:
            ...
            for wave in plan["execution_plan"]["waves"]:
                ...
                for task in wave["tasks"]:
                    wave_assignments[task["task_id"]] = wave_id
            ...
            try:
                subprocess.run(
                    [
                        ...
                        str(issue_num),
                        "--comment",
                        f"✅ Task {task.id} completed via dev-kid",
                    ],
                    check=True,
                    capture_output=True,
                )
...
def show_wave_issues(wave_id: int):
    ...
    for task in wave["tasks"]:
        ...
        if task_id in existing:
            ...
            print(f"   #{issue_num}: {task_id} - {task['instruction'][:50]}...")
        else:
