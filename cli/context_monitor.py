...
def check_context_budget(verbose: bool = False) -> dict:
    ...
    if verbose:
        ...
        print(
            f"   Zone: {zone_info['status']} {zone_info['zone'].upper()} ZONE - {zone_info['level']}"
        )
    ...
    return {"tokens": tokens, **zone_info}
...
def main():
    ...
    parser.add_argument(
        "--check", action="store_true", help="Check context budget and exit with code"
    )
    parser.add_argument(
        "--warn-only", action="store_true", help="Only exit with error if in dumb zone"
    )
