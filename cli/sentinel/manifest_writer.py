...
class ManifestWriter:
    ...
    def _build_summary(self, data: "ManifestData") -> str:
        ...
        cost = (
            f"${tier_result.cost_usd:.3f}"
            if tier_result.cost_usd is not None
            else "$0.000"
        )
        duration = (
            f"{tier_result.duration_sec:.1f}s" if tier_result.duration_sec else "0.0s"
        )
        ...
        if result == "PASS":
            ...
            lines.append(
                ...
                "Check `dev-kid sentinel-health` and `dev-kid.yml` sentinel config."
            )
        else:
            lines.append(
                "⚠ WAVE HALTED\n"
